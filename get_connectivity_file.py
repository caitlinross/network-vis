...
class Network(object):
    ...
    def writeOutputFile(self, filename):
        ...
        with open(filename, "w") as csvf:
            ...
            cw.writerow(["src_gid", "src_type", "src_type_id", "src_group_id", "dest_gid", "dest_type", "dest_type_id", "dest_group_id"])
            for lp in self.all_lps.values():
                for conn in lp.connections:
                    cw.writerow([str(conn.source_obj.gid), str(conn.source_obj.type.value), str(conn.source_obj.typeid),
                                 str(conn.source_obj.group_id), str(conn.dest_obj.gid), str(conn.dest_obj.type.value),
                                 str(conn.dest_obj.typeid), str(conn.dest_obj.group_id)])
...
class DragonflyLP(object):
    def __init__(self, tw_lpid_gid, tw_lpid_df, group_id, lptype, typeid):
...
class Router(DragonflyLP):
    def __init__(self, tw_lpid_gid, tw_lpid_df, group_id, router_id):
        ...
        super().__init__(tw_lpid_gid, tw_lpid_df, group_id, LP_Type.ROUTER, router_id)
...
class Terminal(DragonflyLP):
    def __init__(self, tw_lpid_gid, tw_lpid_df, group_id, terminal_id):
        ...
        super().__init__(tw_lpid_gid, tw_lpid_df, group_id, LP_Type.TERMINAL, terminal_id)
